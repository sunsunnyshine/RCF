...
def generateBlurFlow(gt_fw_flows, gt_bw_flows, fw_flow_agg, fw_residual_adjustment, bw_flow_agg,
    ...
    ## step1: 统一到原图尺寸
    gt_fw_flows = F.interpolate(gt_fw_flows[:, 0], size=origin_img.shape[-2:], mode='bilinear')
    gt_bw_flows = F.interpolate(gt_bw_flows[:, 0], size=origin_img.shape[-2:], mode='bilinear')
    gt_fw_flows = gt_fw_flows * 2.0
    gt_bw_flows = gt_bw_flows * 2.0
    fw_residual_adjustment = F.interpolate(fw_residual_adjustment, size=origin_img.shape[-2:], mode='bilinear')
    bw_residual_adjustment = F.interpolate(bw_residual_adjustment, size=origin_img.shape[-2:], mode='bilinear')
    gt = F.interpolate(gt.unsqueeze(0).float(), size=origin_img.shape[-2:], mode='nearest').squeeze(0)
    ...
    blur_image_objectsharp = process_large_image(origin_img, fw_flow_sharp, bw_flow_sharp, lookup_table,
                                                 flow_range, title_size)
    # output: numpy [B,C,H,W]
    blur_image_objectblur = process_large_image(origin_img, fw_flow_blur, bw_flow_blur, lookup_table,
                                                flow_range, title_size)
